...
def openai_llm(keywords, jif, publisher):
    ...
    system_prompt = (
        f"Act as a journal recommendation assistant. "
        f"Using these provided keywords {keywords}, suggest journals that align with the research topic and meet the following conditions: "
        f"JIF should be strictly greater than {jif}. "
        f"Journals must be from publishers in the provided list. "
        f"Provide results in a clear table format with columns: Journal Name, Publisher, JIF. "
        f"Do not include introductory or ending notes."
        "Context: {context}"
    )
